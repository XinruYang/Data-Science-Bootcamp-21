...
def menu_filtrado(df):
    ...
    distritos, filtro_distrito, tamano, filtro_tamano, operador, filtro_operador = opciones_filtros(df) # Lo de la derecha son funciones
...
def opciones_filtros(df):
    distritos = st.sidebar.selectbox(
        ...
        options=df.DISTRITO.unique().tolist()) # Te cuenta todos los distritos
    filtro_distrito = st.sidebar.checkbox('Quiero filtrar por distrito') # Opción en la que quiero filtrar por distritos
