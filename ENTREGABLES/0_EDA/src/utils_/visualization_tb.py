...
def visualizacion_eight(data, suptitle, x1, x2, x3, x4, y, t1, t2, t3, t4, t5, t6,t7, t8, n, size1, size2):
    ...
    sns.barplot(ax=axes[1, 2], data=data.head(n), x=x3, y=y, color="g", alpha=0.5, label=x3).set(xlabel=None, ylabel=None, yticklabels=[])

    sns.barplot(ax=axes[1, 3], data=data.head(n), x=x4, y=y, color="c", alpha=0.5, label=x4).set(xlabel=None, ylabel=None, yticklabels=[])
    plt.legend()
...
def scatter_geo(df, size, name, color, size_max=None): 
    ...
    title=(f'Map of {name}'), size_max=size_max)
...
def working_time():
    a = pd.DataFrame({'Find Data': np.arange(5)})
    b = pd.DataFrame({'Data Wragling and Data Mining':np.arange(25)})
    c = pd.DataFrame({'Checking Error':np.arange(20)})
    d = pd.DataFrame({'Visualization': np.arange(20)})
    e = pd.DataFrame({'Data Structuring':np.arange(25)})
    f = pd.DataFrame({'Documentation':np.arange(5)})

    tiempo = pd.concat([a,b,c,d,e,f])
    ...
    plt.pie(count_list, labels=labels, autopct='%1.0f%%', explode= (0.02, 0.02,0.02, 0.02,0.02, 0.02))
